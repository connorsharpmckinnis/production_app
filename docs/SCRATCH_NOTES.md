...
## Rehearsal capture / notes — closeout — 2026-09-19

~~Need a Scrooge-ready way to capture notes and Timeline actions during
rehearsal, with TTA suggestions and Director/SM approve…~~ — **Shipped
2026-09-19.** See
[rehearsal-capture-and-approvals.md](feature_plans/rehearsal-capture-and-approvals.md).
Owner still turns on `timeline.approve` for SM and `timeline.suggest` /
`notes.publish` for TTA in Settings after creating those roles. Open button
removed from rehearsal UI; capture stamps by time window. Deferred: house
mode, concurrent-rehearsal picker, hide-suggestions-from-actors, tablet banner.
...
- On the Overview page for a production, I need a little bit of buffer at the bottom of the page so that the bottom-most buttons aren't sitting directly on the bottom of the screen. Just a handful of pixels would be fine. → **Fixed 2026-09-19:** Main content is a block scroll box with `pb-10`. The earlier padding sat on a flex child, so it disappeared once a page was taller than the screen.
...
- Character 'burn-down' chart: A horizontal timeline of the show with color-coded characters occupying rows on the chart, showing what scenes they're in and when they're on/offstage. Perhaps including hover-able icons to display when they have costume, prop, or mic changes. → **Prototype 2026-08-18:** Reports → On-stage chart (E/E presence bars, moment axis, hover, scene reset). Costume/prop/mic icons and break-time still open; see [character-onstage-chart.md](feature_plans/character-onstage-chart.md).
